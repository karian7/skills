...
TDD 대상이다. Daum 서버는 보낸함을 "Sent"(\\Sent), 임시보관함을 "Drafts"(\\Drafts),
...
def imap_select_name(name: str) -> str:
    """IMAP SELECT/EXAMINE 명령에 쓸 메일박스 이름 문자열을 반환한다.

    modified UTF-7로 인코딩한 뒤 공백이 포함된 경우 이중 따옴표로 감싼다.
    IMAP 프로토콜은 공백이 있는 폴더명을 quoted-string으로 전달해야 한다.
    """
