...
Daum/Naver는 SUBJECT 서버 검색 미실측 — 표준 준수 서버라 동작 가능성 있으나
클라이언트측 매칭으로 안전하게 구현.

서버 실측(imap.daum.net): FROM/TO/CC·날짜·플래그·크기 SEARCH만 정확하고, SUBJECT/BODY/TEXT/HEADER는
문법만 수락한 채 항상 0건을 돌려줄 수 있다(BAD가 아니라 OK+빈 결과 — 평문 ASCII
제목 토큰조차 안 잡힐 수 있다). 그래서 서버 SUBJECT에 의존하지 않고 검색을 둘로 나눈다:
...
def _batch_fetch(conn, uids: list[bytes], spec: str) -> dict[bytes, bytes]:
    ...
    일부 서버는 응답을 두 파트로 분리한다:
      part[N]   = (b'SEQ (BODY[...] {size}', b'<header bytes>')  — 헤더 본문
      part[N+1] = b' UID 27500)'                                 — UID + 닫는 괄호
...
def main() -> int:
    ...
    conn.select(imap_select_name(target), readonly=True)
